...
if __name__ == '__main__':
    ...
    # Cross-round Stackelberg state: effort_next from the previous settlement controls the next round's local training intensity.
    ...
    # Structural anomaly detection: persist late-layer parameter snapshots for each client across rounds.
    ...
    for e in range(conf["global_epochs"]):
        ...
        # This round's actual training intensity comes from the previous Stackelberg settlement.
        ...
        # Representative model generation: fuse only the late ResNet-18 layers by default (layer4 + fc).
        ...
        # Use one malicious representative model this round to validate scoring and aggregation down-weighting.
        ...
        # Multi-metric scoring: metrics other than ds_scores can be sampled by config or provided by the caller.
        ...
        # Down-weighted aggregation: clients with Si<=0 are excluded, and the rest are weighted by normalized Si.
        ...
        if positive_score_sum > 0:
            for idx, c in enumerate(candidates):
                ...
                if diff is None:
                    continue
                for name, params in server.global_model.state_dict().items():
                    # state_dict includes integer BatchNorm counter buffers, so only update floating-point weights and buffers.
                    if not params.is_floating_point():
                        continue
                    weight_accumulator[name].add_(diff[name].to(params.dtype) * agg_weight)
        else:
            print("[Aggregation] No clients have positive scores this round; keeping the global model unchanged.")

        # Stackelberg settlement: update reputation and rewards, then output the next round's training intensity.
