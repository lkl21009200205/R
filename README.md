...
This repository contains a local Python simulation of horizontal federated learning for image classification. In addition to the basic FedAvg-style training loop, the code includes client verification, lazy-client incentive scoring, malicious-update simulation, weighted aggregation, and a multi-round Stackelberg incentive mechanism.
...
The code is tested as a standard Python project. Use Python 3.8 or later. The required third-party packages are listed in `requirements.txt`.
...
If you need a specific CUDA build, install the matching PyTorch and torchvision packages for your machine before running the experiment.
...
Run all commands from the repository root unless a command changes directories explicitly. Start a full experiment with the default configuration:
